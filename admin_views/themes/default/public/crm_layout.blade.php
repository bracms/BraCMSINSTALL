...
<div id="bra_app" :class="current_layout" v-cloak style="height: 100%;">
    ...
    <div class="bra-admin-side columns is-mobile is-marginless is-gapless" style="height: 100%;">
        ...
        <!-- level one menu-->
        <div class="menu-list column menu-list-narrow">
  ...
  <i class="fas fa-info-circle"></i>
            ...
            @verbatim
                <div :data-id="menu.id" class="menu-item" :class="{'active' : index==current_group}" v-for="( menu , index ) in menus" @click="switch_group(menu.id)" :data-title="menu.title">

                    <div class="icon-text">
  <span class="icon">
    <i :class="!menu.icon || menu.icon == '#' ? 'fa fa-caret-right': menu.icon"></i>
  </span>
        ...
        @verbatim
            <div class="menu-list-sub column is-narrow menu-list-sub-show" v-if="show_mask">
                ...
                <div v-if="menus[current_group] && current_group !='mine'">
                    <div v-for="menu in menus[current_group].children" class="menu-item-three is-paddingless">

                        <div a @click="open_tab(menu)" :class="{'active' : active_tab_id == menu.id }"
                           v-show="menu.children.length === 0" :data-id="menu.id" :len="menu.children.length"
                           :data-url="menu.url" class="menu-item-sub" >
                            <span class="">
                                    <i :class="!menu.icon || menu.icon == '#' ? 'fa fa-caret-right': menu.icon"></i>
                                    <span class="icon-title">{{menu.title}}</span>
                                </span>
                        </div>
                        <div b v-show="menu.children && menu.children.length > 0" class="menu-item-sub">
                            <i :class="!menu.icon || menu.icon == '#' ? 'fa fa-caret-right': menu.icon" style="margin-right:8px"></i>{{menu.title}}
                        </div>
                    ...
                    .children">
                                <span class="icon-text">
                                    <span class="icon"><i :class="!_menu.icon || _menu.icon == '#' ? 'fa fa-caret-right': _menu.icon"></i></span>
                                    <span class="icon-title">{{_menu.title}}</span>
                                </span>
    ...
    <div class="bra-admin-top">
                ...
                <div class="column">
                    ...
                    <div class="columns is-mobile is-marginless has-text-centered align-items-center is-gapless">
                        <a style="width:65px" class="column is-narrow" @click="modalSign1 = !modalSign1">
                            <i class="fas" :class="{'fas fa-angles-right' : !modalSign1}"></i>
                            <i class="fas" :class="{'fas fa-angles-left' : modalSign1}"></i>
                        ...
                        <div class="column is-gapless open_scroll is-clipped">
                            <div class="columns is-mobile is-marginless align-items-center is-gapless" style="width:100%;">
                                <div class="column is-narrow open_item" v-for="open in opened" :class="{active : active_tab_id == open.id}">
                                    ...
                                    <a class="is-inline-block" @click="active_tab(open)"><i :class="open.icon"></i> {{open.title}}</a>
                                    <i v-if="open.id != 0" @click="close_tab(open)" class="fas fa-close"></i>
                                </div>
                            </div>
                        </div>
        ...
        <!-- right icons -->
            <div class="column is-narrow is-pulled-right has-text-right bra-admin-top-right">
                ...
                @show
                <a @click="clear_cache()">
                    <i class="fas fa-repeat has-text-success"></i>
                </a>

                @section('act_logout')
                    <a bra-mini="confirm" href="{{ make_url('bra_admin/passport/logout')}}">注销</a>
                @show
            </div>
        </div>
