...
@section('footer_js')
    ...
    $lists = BraDB::table('user')->selectRaw("DATE_FORMAT( create_at, '%Y-%m-%d' ) AS 'ct' , count(id) as ids")->groupByRaw("DATE_FORMAT( create_at, '%Y-%m-%d' )")->get();

    $pv_month = BraDB::table('log')->selectRaw("DATE_FORMAT( create_at, '%Y-%m-%d' ) AS 'ct' , count(id) as ids")->groupByRaw("DATE_FORMAT( create_at, '%Y-%m-%d' )")->get();

    $pv_24_hour = BraDB::table('log')->where('create_at' , '>' , Carbon::yesterday())->selectRaw("DATE_FORMAT( create_at, '%d %H' ) AS 'ct' , count(id) as ids")->groupByRaw("DATE_FORMAT( create_at, '%d %H' )")->get();
