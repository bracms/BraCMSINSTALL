define("bra_picker", ['braui'], function () {
    var obj = function (config) {
        ...
        this.createSelect = function (optionData) {
            ...
            var html = '';
            html += '<div class="select layui-input-inline" ' + o.setWidth() + '>';
    ...
    //渲染一个实例
    obj.prototype.render = function (config) {
        var root = this;

        config = $.extend(root.config, config);
        $E = $(config.elem);
        if ($E.length == 0) {
            console.error( ' hint：找不到容器 ' + config.elem);
            return false;
        }
